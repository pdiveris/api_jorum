 ...
 *
 */
namespace Jisc\api\Service\Jorum;
...
class JorumApi implements \JsonSerializable, \IteratorAggregate
{
      ...
      if (is_array($params)) { // initial data in the form if key=>'val'
        ...
        foreach ($params as $key => $value) {
          ...
          if (property_exists($this, $key)) {
            ...
            $className = '\\Jisc\\api\\Service\\Jorum\\' . studly_case($singKey); // studly case means 'some_array' gets converted 'SomeArray'
  ...
  public static function factory($className, $key, $value)
  {
    ...
    if ($key == 'metadata') {
      ...
      return new \Jisc\api\Service\Jorum\Metadata($value);
    }
  ...
  public function findByIdOrHandle($id = '', $options = array())
  {
    ...
    $class = 'Jisc\\api\\Service\Jorum\\' . strtoupper(substr($model, 0, 1)) . substr($model, 1);
  ...
  public function toHtml()
  {
    ...
    while ($iterator->valid()) {
      ...
      echo \Jisc\api\Service\Formatters\Html::get($key, $value, $itemLink);
